...
twr_list, ql_twr_list, dep_list, ql_dep_list, app_list, ql_app_list, ctr_list, ql_ctr_list;
...
void Load_FlightPlan_Interface(double x_, double y_, std::string* strings, bool refresh) {
	...
	if (!fp_frame || refresh) {
		...
		double width = 450.0, x = x_ == -1 ? (CLIENT_WIDTH / 2) - (width / 2) : x_;
		double height = 220.0, y = y_ == -1 ? (CLIENT_HEIGHT / 2) - (height / 2) : y_;
